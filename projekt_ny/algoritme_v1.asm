...
LOOP:
	CALL	COUNTERSEND
	CPI	counter,11		;Første gang over målstreg, bliver den 11, og den skal mappe
	BREQ	JUMPMAIN		;Hopper til main som mapper
	CPI	counter,12		;Anden gang over målstreg, optimerer omgangen
	BREQ	OPTIMER
	CPI	counter,13
	BREQ	STOPBIL
...
OPTIMERLIGEUD:
	LDI	R16,205			;Sætter tophastighed, hvis vi ikke er på den
	OUT	OCR2,R16
	CALL	DELAYLIGEUD
...
REELERIETANDETSVING:
	CPI	ACCdata,90		;Tjekker hvornår vi kommer ud af sving til den anden side (høje sving)
	BRLO	REELUDAFSVING2
	JMP	TJEKREEL
...
REELSKIP:
	...
	JMP	TJEKREEL
REELSTADIGISVING2:
	CPI	ACCdata,90		;Hvis 2 målinger ikke siger den er sving, clear den sving
...
DELAYBREMS:
	LDI	R19,42			;skal være det samme som ACCreg fra start
	SUB	R19,ACCreg
	;LDI	R16,2
	;MUL	R16,R19
